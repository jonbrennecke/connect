...
/**
 * All the AJAX functions one plugin could ever want!
 *
 * @package WordPress
 * @subpackage Social
 * @since Social 1.0
					...
					<?php 
						...
						foreach ( $tooltips[ $_POST['sectionName'] ] as $key => $tip) {
							...
							if ( $key + 1 == count($tooltips[ $_POST['sectionName'] ]) ) {
								...
								do_settings_fields( 'social_settings_page', $_POST['sectionName'] . '_section' );
...
function twitter_profile(){
	...
	// make sure we're not returning an error message
 ...
 *
 * In the case of Twitter, which uses OAuth 1.0, instead of returning a URL to a confirmation 
 * page, we're just storing the access token in the database.
 *
...
function login_url( $section ) {
	switch ( $section ) {
		...
		case 'twitter':
			...
			/**
			* Twitter uses OAuth 1.0 for authentication, and doesn't require a login page or redirect. So 
			* the process here is a little different than the others. All we need to do is get an access 
			* token from Twitter and save it to the database.
			*
			*/

			$api = new \Twitter_API( array(
				'api-key' => get_option( 'twitter-api-key' ),
				'api-secret' => get_option( 'twitter-api-secret' ),
				'access-token' => get_option( 'twitter-access-token' )
			));
			...
			if ( $token ) {
				update_option( 'twitter-access-token', $token );
			}
